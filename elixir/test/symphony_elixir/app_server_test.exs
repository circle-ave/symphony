defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server treats codex error notifications as turn failures" do
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case "$count" in
          ...
          3)
            ...
            ;;
          4)
            printf '%s\\n' '{"method":"error","params":{"message":"model exploded","code":"oops"}}'
            exit 0
            ;;
          *)
            exit 0
      ...
      issue = %Issue{
        ...
        identifier: "MT-ERR",
        title: "Handle Codex error notification",
        description: "Ensure app-server error frames fail the turn",
        state: "In Progress",
      ...
      assert {:error, {:turn_error, %{"code" => "oops", "message" => "model exploded"}}} =
               AppServer.run(workspace, "Trigger error frame", issue, on_message: on_message)

      assert_received {:app_server_message,
                       %{
                         event: :turn_error,
                         details: %{"code" => "oops", "message" => "model exploded"}
                       }}
    after
      File.rm_rf(test_root)
    end
  ...
  test "app server auto-approves MCP tool approval elicitations when approval policy is never" do
    test_root =
      Path.join(
        System.tmp_dir!(),
    ...
    try do
      ...
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-mcp-tool-approval.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEX_TRACE")
      ...
      while IFS= read -r line; do
        ...
        case "$count" in
          ...
          4)
            ...
            printf '%s\\n' '{"id":110,"method":"mcpServer/elicitation/request","params":{"_meta":{"codex_approval_kind":"mcp_tool_call","source":"connector","connector_name":"Linear","tool_title":"save_comment"},"message":"Allow Linear to run tool \\"linear_save_comment\\"?","mode":"form","requestedSchema":{"type":"object","properties":{}},"serverName":"codex_apps"}}'
            ;;
          5)
            printf '%s\\n' '{"method":"turn/completed"}'
            exit 0
      ...
      issue = %Issue{
        ...
        description: "Allow app tool approval elicitations unattended",
        state: "In Progress",
        url: "https://example.org/issues/MT-189",
        labels: ["backend"]
      }

      assert {:ok, _result} = AppServer.run(workspace, "Handle MCP tool approval", issue)
    ...
    try do
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 tool_names = Enum.map(dynamic_tools, & &1["name"])
                 linear_tool = Enum.find(dynamic_tools, &(&1["name"] == "linear_graphql"))
                 jira_tool = Enum.find(dynamic_tools, &(&1["name"] == "jira_issue_attachments"))

                 payload["id"] == 2 and
                   tool_names == ["linear_graphql", "jira_issue_attachments"] and
                   get_in(linear_tool, ["inputSchema", "required"]) == ["query"] and
                   get_in(jira_tool, ["inputSchema", "required"]) == ["issue"] and
