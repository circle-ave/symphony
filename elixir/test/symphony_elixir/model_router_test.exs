defmodule SymphonyElixir.ModelRouterTest do
  ...
  test "comment reply mode skips the router and uses a compact fast command" do
    ...
    assert route.command =~ "--config features.memories=false"
    assert route.command =~ "--config features.plugins=false"
    assert route.command =~ "--config features.shell_tool=false"
    assert route.command =~ "--config features.unified_exec=false"
